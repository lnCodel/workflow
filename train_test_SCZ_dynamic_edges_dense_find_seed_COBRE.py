...
from models_dynamic_edges_dense_k_hup_neighbors_COBRE import init_model_dict, init_optim
...
def prepare_trte_data(data_folder, view_list,fMRI_type,data_subfolder,data_num,atlas):
    label_file="labels_fold.csv"
    source_folder = data_folder
    a = os.path.join(source_folder,label_file)
    label_csv = pd.read_csv(os.path.join(source_folder,label_file))
    if fMRI_type == 1:
        data_fMRI_csv = pd.read_csv(
            os.path.join(source_folder, "fMRI", atlas, "ReHo_VMHC_fALFF_ALFF", "VMHC", "VMHC_features.csv"), header=None)
    elif fMRI_type == 2:
        data_fMRI_csv = pd.read_csv(
            os.path.join(source_folder, "fMRI", atlas, "ReHo_VMHC_fALFF_ALFF", "ReHo", "ReHo_features.csv"),header=None)
    elif fMRI_type == 3:
        data_fMRI_csv = pd.read_csv(
            os.path.join(source_folder, "fMRI", atlas, "ReHo_VMHC_fALFF_ALFF", "ALFF", "ALFF_features.csv"),header=None)
    elif fMRI_type == 4:
        data_fMRI_csv = pd.read_csv(
            os.path.join(source_folder, "fMRI", atlas, "ReHo_VMHC_fALFF_ALFF", "fALFF", "fALFF_features.csv"),header=None)
    elif fMRI_type == 5:
        data_fMRI_csv = pd.read_csv(
            os.path.join(source_folder, "fMRI", atlas, "fc_fisher",  "feature_fc.csv"))
    data_sMRI_csv = pd.read_csv(
        os.path.join(source_folder, "sMRI", atlas, "GMV", "GMV_features.csv"), header=None)
    # data_sMRI_csv = pd.read_csv(
    #     os.path.join(source_folder, "fMRI", "aal_116","fc_fisher",  "feature_fc.csv"))
    data_DTI=[]
    if data_subfolder=="GAD":
        data_DTI_csv = pd.read_csv(
            os.path.join(source_folder, "DTI", "AllAtlasResults", "WMlabelResults_FA.csv"), header=None)
        data_DTI = data_DTI_csv.values[:, 1:]
    label = np.array(label_csv.values[:, 3:4]).astype(int)# 所有数据的label
...
def test_data(fMRI, sMRI,DTI,types):
    label_file = "labels_test.csv"
    source_folder = "/homeb/lining/Code/depression1/depression3/"
    label_csv = pd.read_csv(os.path.join(source_folder, label_file))
    data_fMRI_csv = pd.read_csv(fMRI,header=None)
    data_sMRI_csv = pd.read_csv(sMRI,header=None)
    data_fMRI = data_fMRI_csv.values[:, 1:]
    data_sMRI = data_sMRI_csv.values[:, 1:]
    ids = data_fMRI_csv.values[:, 0:1]
    label = np.array(label_csv.values[:, 3:4]).astype(int)  # 所有数据的label
    if types == "GAD":
        data_DTI_csv = pd.read_csv(DTI, header=None)
        data_DTI = data_DTI_csv.values[:, 1:]
        return data_fMRI, data_sMRI, data_DTI
    else:
        return data_fMRI, data_sMRI, None,ids, label
...
def train_test(data_folder, view_list, num_class,
    ...
    if data_folder == '/homeb/lining/Code/depression1/depression3/':
    ...
    label,data_fMRI, data_sMRI, data_DTI,names = prepare_trte_data(data_folder, view_list,
                                                                   fMRI_type,data_subfolder,data_num,atlas)
    new_name = []
    for na in names:
        new_name.append(na)
    train_val = []
    ...
    #读取测试集
    data_fMRI_test, data_sMRI_test, data_DTI_test, test_names, label_test = test_data(test_fMRI, test_sMRI, test_DTI,
                                                              data_subfolder)


    for i, na in enumerate(test_names):
        new_name.append(str(na[0]) +"_test")


    test_sum = len(data_fMRI_test)
    test = []
    for i in range(test_sum):
        test.append(i + len(label))
    data_fMRI = np.append(data_fMRI,data_fMRI_test,axis=0)
    data_sMRI = np.append(data_sMRI,data_sMRI_test,axis=0)
    if data_subfolder == 'GAD':
        for i in range(test_sum):
            label = np.append(label, [label_test[i]], axis=0)
    else:
        for i in range(test_sum):
            label = np.append(label, [label_test[i]], axis=0)
    label_int=[]
    ...
    for fold_repeat,N_SEED in enumerate(N_SEED_all):
        ...
        for train_index, test_index in skf.split(label[0 : len(label_int) - len(data_fMRI_test)],
            ...
            data_mat_list = []
            for i in range(num_view):
                data_mat_list.append(np.concatenate((data_tr_list[i], data_te_list[i], data_va_list[i]), axis=0))
            data_tensor_list = []
            ...
            adj_tr_list, adj_te_list = gen_trte_adj_mat(data_tr_list, data_trteva_list, trte_idx, adj_parameter)
            n = len(test_names)
            ...
            for na in names_test:
                base_path = save_Intermediate_results(v=new_name, adj_list=adj_te_list, num=split_time, type="original",
                                                  type_shujv=data_subfolder, base_path=os.path.join("/homeb/lining/Code/depression1/depression3/results/", atlas), subject=na)
            ...
            for epoch in range(num_epoch+1):
                ...
                if epoch % test_inverval == 0:
                    ...
                    if num_class == 2:
                        ...
                        sen=tp/(tp+fn)
                        spe=tn/(tn+fp)
                        auc_test = roc_auc_score(labels_trte[trte_idx["te"]], te_prob[:, 1])
                        f1_scor=f1_score(labels_trte[trte_idx["te"]], te_prob.argmax(1))
                        fpr, tpr, threshold = roc_curve(labels_trte[trte_idx["te"]], te_prob[:, 1])
                        if (acc_te_best < acc):
                            _, va_prob, adj_test = test_epoch(data_trteva_list, adj_te_list, trte_idx["va"],
                                                                     model_dict,
                                                                     adj_parameter)
                            tn_va, fp_va, fn_va, tp_va = confusion_matrix(labels_trte[trte_idx["va"]], va_prob.argmax(1)).ravel()
                            auc_va = roc_auc_score(labels_trte[trte_idx["va"]], va_prob[:, 1])
                            acc_va = accuracy_score(labels_trte[trte_idx["va"]], va_prob.argmax(1))
                            sen_va = tp_va / (tp_va + fn_va)
                            spe_va = tn_va / (tn_va + fp_va)
                            save_model_dict(models_dir, model_dict,fold_repeat,split_time) #当前重复次数，当前次数内第几折
                            acc_te_best = acc
                            sen_te_best=sen
                            spe_te_best=spe
                            auc_te_best = auc_test
                            f1_te_best = f1_scor
                            epoch_best = epoch
                            b = te_prob1[:, 0]
                            ...
                            best_adj = adj_test
                        print(f"Val: acc_best: {acc_te_best} sen_best:{sen_te_best} spe_best {spe_te_best} auc_best {auc_te_best}")
                        #print("SEED: {:d}".format(N_SEED))
                        #print("fold_repeat: {:d}".format(fold_repeat))
                        #print("split time: {:d}".format(split_time))
                        #print("Test ACC: {:.3f}".format(acc))
                        #print("Test SEN: {:.3f}".format(sen))
                        #if(spe == 0):
                        #    print("Test AUC: {:.3f}".format(0))
                        #print("Test SPE: {:.3f}".format(spe))
                        #if(spe != 0):
                    ...
                    if epoch == num_epoch:
                        ...
                        acc_list_va.append(acc_va)
                        auc_list_va.append(auc_va)
            n = len(test_names)
            names_test = new_name[-n:]
            for na in names_test:
                base_path = save_Intermediate_results(v=new_name, adj_list=best_adj, num=split_time, type="Intermediate",
                                                  type_shujv=data_subfolder, base_path=os.path.join("/homeb/lining/Code/depression1/depression3/results/", atlas),subject=na)
        print(f"! Val: acc_best_mean: {np.mean(acc_list)} sen_best_mean:{np.mean(sen_list)}"
              f"spe_best {np.mean(spe_list)} auc_best {np.mean(auc_list)}")
        print("")
        max_value = max(acc_list_va)
        # 使用index方法找到最大值的下标
        max_index = acc_list_va.index(max_value)
        print(f"! Test: acc_best_mean: {acc_list_va[max_index]}, auc_best {auc_list_va[max_index]}")
        print("")
        source_folder = os.path.join(data_folder, data_subfolder, data_num,"models_"+atlas, model_folder,type_folder)
        seedfile=open(os.path.join("/homeb/lining/Code/depression1/depression3/results/", atlas, "s.txt"),"a+")
        for i in range(1, len(data_fMRI_test) + 1):
            seedfile.write(f"The predicted probability of patient {new_name[-i]} being depressed is  %.4f\n" % (test_pos[max_index][-i]))
        seedfile.close()
        ...
        seedcsv.close()
        print("The image is saved in {/homeb/lining/Code/depression1/depression3/results/}")
        a123 = os.path.join("/homeb/lining/Code/depression1/depression3/results/", atlas, "s.txt")
        print(f"The prediction results are saved in {a123}")


def model_svm(data_folder, view_list, num_class,
               lr_e_pretrain, lr_e, lr_c,
               num_epoch_pretrain, num_epoch,fold,
               fMRI_type,fold_repeat_all,data_subfolder,
               N_SEED_all,adj_parameter,dim_he_list,data_num,atlas,model_folder,type_folder
    ...
    data_fMRI_test, data_sMRI_test, data_DTI_test, test_names = test_data(test_fMRI, test_sMRI, test_DTI,
                                                                          data_subfolder)
    best_rfc = RandomForestClassifier(max_depth=400,
                                      min_samples_split=10,
                                      n_estimators=10, random_state=123, criterion="gini")

    best_rfc.fit(data_fMRI, label)  # 模型拟合
    ca = best_rfc.predict(data_fMRI_test)
    seedfile = open(os.path.join("/homeb/lining/Code/depression1/depression3/results_RF/", atlas, "s.txt"), "a+")
    for i in range(1, len(data_fMRI_test) + 1):
        if(ca[i - 1] == 1):
            seedfile.write(f"The Patient {test_names[i - 1]} may have depression.\n")
        else:
            seedfile.write(f"The patient {test_names[i - 1]} is in good health.\n")